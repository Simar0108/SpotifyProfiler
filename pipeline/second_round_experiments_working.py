#!/usr/bin/env python3
"""
Second Round Hyperparameter Experiments - WORKING VERSION

This script uses the EXACT SAME model architecture as your successful first round.
The problem was using a complex neural network instead of simple dot product similarity.

FIXES:
- Uses simple dot product similarity (same as first round)
- Same model architecture - NO hidden layers
- Proper data splitting
- Realistic loss progression expected
"""
...
class Item2VecModel(nn.Module):
    """Item2Vec model using EXACT SAME architecture as successful first round"""
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, dropout_rate=0.1):
        ...
        # Embedding layers - separate for track and context (SAME as first round)
        self.track_embeddings = nn.Embedding(vocab_size, embedding_dim)
        self.context_embeddings = nn.Embedding(vocab_size, embedding_dim)
        
        # CRITICAL: Use the SAME architecture as first round - NO hidden layers!
        # Just apply hidden layers to embeddings, then dot product similarity
    ...
    def forward(self, pos_track, context_track, neg_tracks):
        """Forward pass with EXACT SAME logic as first round"""
        # Get embeddings
        ...
        # Apply hidden layers (SAME as first round)
        pos_emb = self.hidden(pos_emb)
        context_emb = self.hidden(context_emb)
        neg_emb = self.hidden(neg_emb)
        
        # CRITICAL: Use the SAME similarity calculation as first round
        # Simple dot product similarity - NO neural network output layer!
...
class ExperimentManager:
    ...
    def __init__(self, checkpoint_file="second_round_checkpoint_working.json"):
...
def create_weight_decay_experiments():
    ...
    base_config = {
        ...
        'batch_size': 1024,
        'negative_samples': 20,
        'optimizer': 'adam',  # FIXED: Added missing optimizer field
        'dropout_rate': 0.1,
        'lr_scheduler': None,
        'epochs': 15,
        'patience': 5,
...
def run_single_experiment(config, experiment_name, db_path, max_samples=100000, k_folds=3):
    ...
    # CRITICAL FIX: Create dataset ONCE, then split it properly
    logger.info("📊 Creating dataset for cross-validation...")
    dataset = CoOccurrenceDataset(db_path, max_samples, config['negative_samples'])
    
    # K-fold cross-validation on the SAME dataset
    kf = KFold(n_splits=k_folds, shuffle=True, random_state=42)
    
    for fold in range(1, k_folds + 1):
        logger.info(f"🔄 Fold {fold}/{k_folds}")
        
        # Split the SAME dataset - this is the key fix
        ...
        # Create data loaders from the split indices
        ...
        for epoch in range(config['epochs']):
            ...
            for batch in train_loader:
                ...
                pos_sim, neg_sim = model(
                    batch['pos_track'], 
                    batch['context_track'], 
                    batch['neg_tracks']
                )
                
                # Use the SAME loss function as successful first round
            ...
            with torch.no_grad():
                for batch in val_loader:
                    ...
                    # Same loss function for validation
                    pos_loss = -F.logsigmoid(pos_sim).mean()
                    neg_loss = -F.logsigmoid(-neg_sim).mean()
...
def main():
    ...
    logger.info("🚀 Starting Second Round Hyperparameter Experiments (WORKING VERSION)")
    logger.info("=" * 60)
    ...
    if all_results:
        ...
        best_config_file = "best_second_round_config_working.json"
        ...
        plt.title('Second Round Hyperparameter Tuning Results (WORKING VERSION)')
        ...
        plt.savefig('second_round_results_working.png', dpi=300, bbox_inches='tight')
        logger.info("📊 Summary plot saved: second_round_results_working.png")
