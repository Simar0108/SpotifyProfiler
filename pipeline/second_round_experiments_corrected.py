#!/usr/bin/env python3
"""
Second Round Hyperparameter Experiments - CORRECTED VERSION

This script fixes the critical data splitting issue that was causing unrealistic loss values.
The problem was that a new dataset was created for each fold, leading to data leakage.

FIXES:
- Proper data splitting: one dataset, split across folds
...
class Item2VecModel(nn.Module):
    """Simplified Item2Vec model based on first round success"""
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, dropout_rate=0.1):
        ...
        # Simple hidden layer - much simpler than before
    ...
    def forward(self, pos_track, context_track, neg_tracks):
        ...
        for i in range(neg_tracks.size(1)):
            ...
            neg_score = neg_score.squeeze(-1)  # Ensure proper shape
            neg_scores.append(neg_score)
        
        neg_scores = torch.stack(neg_scores, dim=1)
        
        return pos_score.squeeze(-1), neg_scores

class ExperimentManager:
    """Manages experiment execution with checkpointing and resume capability"""
    
    def __init__(self, checkpoint_file="second_round_checkpoint_corrected.json"):
...
def main():
    ...
    logger.info("🚀 Starting Second Round Hyperparameter Experiments (CORRECTED)")
    logger.info("=" * 60)
    ...
    if all_results:
        ...
        best_config_file = "best_second_round_config_corrected.json"
        ...
        plt.title('Second Round Hyperparameter Tuning Results (CORRECTED)')
        ...
        plt.savefig('second_round_results_corrected.png', dpi=300, bbox_inches='tight')
        logger.info("📊 Summary plot saved: second_round_results_corrected.png")
