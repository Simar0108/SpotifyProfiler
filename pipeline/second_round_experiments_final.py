#!/usr/bin/env python3
"""
Second Round Hyperparameter Experiments - FINAL CORRECTED VERSION

This script uses the EXACT SAME loss function and model architecture as your successful first round.
The problem was using binary_cross_entropy instead of logsigmoid loss.

FIXES:
- Uses logsigmoid loss (same as first round)
...
class Item2VecModel(nn.Module):
    ...
    def __init__(self, vocab_size, embedding_dim, hidden_dim, dropout_rate=0.1):
        ...
        # Embedding layers - separate for track and context
        self.track_embeddings = nn.Embedding(vocab_size, embedding_dim)
        self.context_embeddings = nn.Embedding(vocab_size, embedding_dim)
        
        # Hidden layer - same as first round
    ...
    def forward(self, pos_track, context_track, neg_tracks):
        ...
        return pos_score.squeeze(-1), neg_scores.squeeze(-1)

class ExperimentManager:
    """Manages experiment execution with checkpointing and resume capability"""
    
    def __init__(self, checkpoint_file="second_round_checkpoint_final.json"):
    ...
    for fold in range(1, k_folds + 1):
        ...
        for epoch in range(config['epochs']):
            ...
            for batch in train_loader:
                ...
                # CRITICAL FIX: Use the SAME loss function as successful first round
                # Negative log likelihood (logsigmoid) instead of binary cross-entropy
            ...
            # Logging - FIXED: Log every epoch, not just every 5
...
def main():
    ...
    logger.info("🚀 Starting Second Round Hyperparameter Experiments (FINAL CORRECTED)")
    logger.info("=" * 60)
    ...
    if all_results:
        ...
        best_config_file = "best_second_round_config_final.json"
        ...
        plt.title('Second Round Hyperparameter Tuning Results (FINAL CORRECTED)')
        ...
        plt.savefig('second_round_results_final.png', dpi=300, bbox_inches='tight')
        logger.info("📊 Summary plot saved: second_round_results_final.png")
